...
const validateSpot = [
  ...
  check("rooms").isInt({ min: 0, max: 50 }).withMessage("Rooms must be between 0 and 50"),
  check("beds").isInt({ min: 0, max: 50 }).withMessage("Beds must be between 0 and 50"),
  check("baths").isInt({ min: 0, max: 50 }).withMessage("Baths must be between 0 and 50"),
  check("price")
    .exists({ checkFalsy: true })
    .isFloat({ min: 0 })
